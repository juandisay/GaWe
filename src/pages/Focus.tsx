...
export const Focus = () => {
  ...
  const toggleMusic = async () => {
    if (isMusicPlaying) {
      ...
      if (settings.musicFilePath) {
        await invoke('play_music', { filePath: settings.musicFilePath });
  ...
  return (
    <div className={`h-screen w-screen relative overflow-hidden transition-colors duration-1000 ${bgColor} text-white flex flex-col`}>
      ...
      <div className="absolute inset-0 pointer-events-none">
        <motion.div
          ...
          className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[80vh] h-[80vh] rounded-full blur-[150px] ${isBreak ? 'bg-green-900/20' : 'bg-blue-900/20'}`}
        />
      </div>
        ...
        <div className="flex gap-4">
          ...
          <div className="relative">
            <Button
              ...
              className={`hover:bg-white/10 ${showMusicControls ? 'text-white bg-white/10' : 'text-gray-400'}`}
            >
              <Music size={24} />
            </Button>
              ...
              {showMusicControls && (
                ...
                >
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <button onClick={toggleMusic} className="p-2 bg-white/10 hover:bg-white/20 rounded-full transition-colors">
                  ...
                  <div className="mt-3 text-xs text-gray-500 truncate flex items-center gap-2">
                    <Music size={12} />
                    {settings.musicFilePath ? settings.musicFilePath.split('/').pop() : 'No file selected'}
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
          ...
          <Button variant="ghost" size="icon" onClick={toggleFullscreen} className="hover:bg-white/10 text-gray-400 hover:text-white cursor-pointer">
      ...
      <div className="flex-1 flex flex-col items-center justify-center z-0 relative">
        ...
        >
          <div className={`mb-6 inline-flex px-4 py-1.5 rounded-full bg-white/5 border border-white/5 text-sm font-medium tracking-widest uppercase ${accentColor}`}>
            {timerState.is_break ? 'Time to Recharge' : 'Focus Session'}
          </div>

          <h2 className="text-3xl font-medium text-gray-300 mb-20 opacity-80">{timerState.current_task_name}</h2>
          ...
          <div className="flex gap-6 justify-center items-center">
            <button
              onClick={async () => {
                if (timerState.is_running) {
                  const confirmed = await ask('This will end your current session. Are you sure?', {
                    title: 'Stop Session',
                    kind: 'warning'
                  });
                  if (!confirmed) return;
            ...
            <button
              onClick={toggleTimer}
              className={`p-8 rounded-[2rem] transition-all duration-300 hover:scale-105 active:scale-95 shadow-xl shadow-white/5 cursor-pointer ${isBreak ? 'bg-green-500 text-black hover:bg-green-400' : 'bg-white text-black hover:bg-gray-200'}`}
            >
              {timerState.is_running ? <Pause size={48} fill="currentColor" /> : <Play size={48} fill="currentColor" className="ml-2" />}
            </button>
